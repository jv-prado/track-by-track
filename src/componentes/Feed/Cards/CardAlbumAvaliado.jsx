...
const CardAlbumAvaliado = ({ album, setAlbumSelecionado, modo = "grade" }) => {
  ...
  const percentualFormatado =
    progressoAvaliacao && typeof progressoAvaliacao.percentual === "number"
      ? Math.floor(progressoAvaliacao.percentual)
      : 0;
        ...
        >
          ...
          <div className="flex-grow min-w-0 mx-2 flex flex-col justify-center">
              ...
              <div>
                ...
                <p className="text-verde-destaque text-xs md:text-sm line-clamp-1 font-medium truncate pr-1">
                  {info.artists && info.artists.length > 0
                    ? info.artists.map((a) => a.name || "").join(", ").length >
                      28
                      ? info.artists
                          .map((a) => a.name || "")
                          .join(", ")
            ...
            <div className="flex items-center mt-2 md:mt-3 gap-2 justify-between">
              {/* Progresso compacto */}
              {progressoAvaliacao && typeof progressoAvaliacao === "object" && (
              ...
              {info &&
                ((info.external_urls && info.external_urls.spotify) ||
                  ...
                  <a
                    href={
                      info && info.external_urls && info.external_urls.spotify
        ...
        <div className="relative w-full">
          <div className="aspect-square overflow-hidden w-full">
            ...
            imgOk ? (
              ...
              <div className="w-full h-full bg-cinza flex items-center justify-center"></div>
            )}
          </div>
          ...
          <div className="space-y-2 flex-grow">
            <div>
              <h3
                ...
                title={info && info.name ? info.name : ""}
              >
                {info &&
                info.name &&
                typeof info.name === "string" &&
                info.name.length > 25
                  ? `${info.name.substring(0, 25)}...`
                  : (info && info.name) || ""}
              </h3>
              <p className="text-sm text-verde-destaque truncate">
                {info &&
                info.artists &&
                Array.isArray(info.artists) &&
                info.artists.length > 0
                  ? info.artists.map((a) => (a && a.name) || "").join(", ")
                  : t("albumCard.unknownArtist")}
              </p>
            </div>

            {/* Barra de progresso */}
            {!erro && album && !album.erro && progressoAvaliacao && (
              <div className="mt-2">
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="text-gray-400 text-[10px]">
                    {progressoAvaliacao.avaliadas || 0}/
                    {progressoAvaliacao.total || 0} ({percentualFormatado}%)
                  </span>
                </div>
                <div className="w-full h-1.5 bg-cinza rounded-full overflow-hidden">
                  <div
                    className={`h-full transition-all duration-300 ease-in-out ${
                      typeof percentualFormatado === "number" &&
