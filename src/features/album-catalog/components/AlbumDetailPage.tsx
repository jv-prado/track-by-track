...
import { Music, Pencil, ArrowLeft, Share2, Eye, EyeOff } from "lucide-react";
...
import { StarRating } from "@/features/ranking/components/StarRating";
import { ReviewForm } from "@/features/ranking/components/ReviewForm";
import { FavoriteWorstPicker } from "@/features/ranking/components/FavoriteWorstPicker";
import { RankingActions } from "@/features/ranking/components/RankingActions";
import { AlbumStatsSection } from "@/features/discovery/components/AlbumStatsSection";
import { AlbumReviewsList } from "@/features/discovery/components/AlbumReviewsList";
...
export function AlbumDetailPage() {
  ...
  const handleShare = async () => {
    ...
    const url = `${window.location.origin}/perfil/${currentUser.id}/album/${albumId}`;
    try {
      if (navigator.share) {
  ...
  return (
    <div className="w-full">
      <div className="relative z-0 -mx-6 -mt-6 px-6 pt-6 sm:-mx-10 sm:-mt-10 sm:px-10 sm:pt-10">
        <div
          aria-hidden
          className="absolute inset-0 overflow-hidden bg-gradient-to-b from-roxo-vivo/40 via-roxo-escuro/50 to-transparent"
        />
