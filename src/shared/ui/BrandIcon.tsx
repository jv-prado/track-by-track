/**
 * Logos de marca (Spotify/YouTube/Apple Music) via SVG inline.
 * Lucide não tem ícone de marca (removidos do pacote); react-icons foi
 * removido do projeto (CLAUDE.md 5.1) — sem reintroduzir. Path = logo oficial simplificado.
 */
type BrandIconProps = {
  size?: number;
  className?: string;
...
export function SpotifyIcon({ size = 14, className }: BrandIconProps) {
  return (
...
export function YoutubeIcon({ size = 14, className }: BrandIconProps) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} className={className} fill="currentColor" aria-hidden="true">
...
export function AppleMusicIcon({ size = 14, className }: BrandIconProps) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} className={className} fill="currentColor" aria-hidden="true">
