  ...
  return (
    <View>
      <View className="mb-3 flex-row items-center gap-2">
        ...
        <Text className="font-semibold text-white">Comentários</Text>
      </View>

      <Card className="gap-4">
        <View className="flex-row gap-2">
          <Input value={newText} onChangeText={setNewText} placeholder="Escreva um comentário..." containerClassName="flex-1" />
        ...
        <View className="gap-3">
            ...
            <View key={comment.id} className="gap-1 border-b border-white/5 pb-2">
              <View className="flex-row items-center justify-between">
                <View className="flex-row items-center gap-2">
                  ...
                  {comment.editedAt && <Text className="text-xs text-gray-500">(editado)</Text>}
                </View>
                {comment.authorId === currentUserId && (
                  <View className="flex-row items-center gap-3">
                    ...
                    <Pressable onPress={() => handleDelete(comment.id)} disabled={deleteComment.isPending}>
          ...
          {data?.data.length === 0 && <Text className="text-sm text-gray-500">Sem comentários ainda.</Text>}
        </View>
      </Card>
    </View>
  );
